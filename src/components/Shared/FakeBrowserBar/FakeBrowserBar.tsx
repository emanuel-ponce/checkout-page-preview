...
function FakeBrowserBar({ url }: FakeBrowserBarProps) {
  const FakeButton = () => <div className="mr-[6px] w-2 h-2 rounded-full bg-gray/10" />;
  const AddressBar = ({ url }: { url: FakeBrowserBarProps['url'] }) => (
    <div className="mx-[6px] flex items-center justify-center rounded-xl bg-gray/5 h-[20px] flex-1 text-[8px] font-semibold gap-2 text-gray">
      <MdLock />
      <span>{url}</span>
    </div>
  );
